...
export async function POST(req: NextRequest) {
  try {
    const { cedulaOCorreo, canal, telefono, codigoOTP, nombreDocente } = await req.json();
      ...
      try {
        const resCorreo = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            html: `
              <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; rounded-2xl: 12px; background: #ffffff;">
                <div style="background: #047857; padding: 16px; border-radius: 8px; text-align: center; color: white;">
                  <h2 style="margin: 0; font-size: 20px;">Formación Tecnológica • MEP</h2>
        ...
        console.error("Error al despachar correo vía Resend:", err);
      }
    }
    ...
    if (evolutionUrl && evolutionApiKey && evolutionInstance && canal === "whatsapp" && telefonoDestino) {
      try {
        ...
        console.error("Error al despachar mensaje vía Evolution API:", err);
      }
    }
    ...
    return NextResponse.json({
      exito: true,
      mensaje:
        canal === "whatsapp" && despachoWhatsAppExitoso
          ? "Código despachado exitosamente por WhatsApp."
          : despachoCorreoExitoso
          ? "Código enviado exitosamente a tu correo oficial MEP."
